...
function sholatAll(kota) {
    return new Promise(async (resolve, reject) => {
        ...
        lop= cheerio.load(html)
        let data = []
        lop('table[class="table-index-jadwal"] > tbody > tr').each(function (i, e) {
            data.push({
        ...
        }); resolve({ creator: '@fanky', status: true, data })
    })
}
...
module.exports = (kota) => {
    return new Promise(async (resolve, reject) => {
        ...
        if (!sholat.status) return resolve({ creator: '@fanky', status: false })
        let now = moment(new Date * 1).format('DD-MM-yy')
        let data = sholat.data.find(v => v.tanggal == now)
        if (typeof data == 'undefined') return resolve({ creator: '@fanky', status: false })
        resolve({ creator: '@fanky', status: true, data })
    })
}